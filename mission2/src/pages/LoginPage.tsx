        ...
        <div className='flex flex-col items-center justify-center h-full gap-4'>
            <div className='flex flex-col gap-3'>
                <div className='flex items-center justify-center gap-2 relative w-[300px] mx-auto mb-5'>
                    ...
                    <span className='text-xl font-bold text-blue-400'>로그인</span>
                </div>
                <input 
                    {...getInputProps('email')}
                    className={`border border-[#ccc] w-[300px] p-[10px] focus:border-[#807bff] rounded-sm 
                        ${errors?.email && touched?.email ? "border-blue-500 bg-blue-200" : "border-gray-300 bg-gray-50"}`}
                    type={"email"}
                    placeholder={"이메일"}
                />
                ...
                <input 
                    ...
                    className={`border border-[#ccc] w-[300px] p-[10px] focus:border-[#807bff] rounded-sm 
                        ${errors?.password && touched?.password ? "border-blue-500 bg-red-200" : "border-gray-300 bg-gray-50"}`}
                    type={"password"}
                    placeholder={"비밀번호"}
                />
                {errors?.password && touched?.password && (<div className='text-blue-500 text-sm'>{errors.password}</div>)}
                <button 
                    type='button'
                    onClick={handleSubmit}
                    disabled={isDisabled}
                    className='w-full bg-blue-500 text-white py-3 rounded-md text-lg font-medium hover:bg-blue-600 transition-colors cursor-pointer disabled:bg-slate-400'
                >
                    로그인
                </button>
                <button 
